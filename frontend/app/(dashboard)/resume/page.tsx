                  ...
                  <div ref={feedback.scoreRef} className={cn("lg:col-span-12 liquid-glass p-10 rounded-[48px] border border-white/5 relative overflow-hidden transition-all", feedback.isHighlighting && "score-highlight")}>
                    <div className="flex items-center gap-4 mb-8">
                      ...
                      <div className="ml-auto text-right">
                        ...
                        <span className={cn("text-[10px] font-black uppercase tracking-widest px-3 py-1 rounded-full mt-1 inline-block", normalizedAnalysis.jd_match.jd_match_score >= 60 ? "bg-emerald-500/10 text-emerald-400" : normalizedAnalysis.jd_match.jd_match_score >= 30 ? "bg-amber-500/10 text-amber-400" : "bg-red-500/10 text-red-400")}>
                    ...
                    <p className="text-[10px] font-mono text-zinc-600 mb-6">Tech: {normalizedAnalysis.jd_match.tech_matched_count} / {normalizedAnalysis.jd_match.tech_required_count} matched</p>

                    {readiness && (
                      <div className="mb-6">
                        <ReadinessScoreCard readiness={readiness.score} label={readiness.label} delta={readiness.delta} />
                      </div>
                    )}
                      ...
                      <div className="space-y-3">
                        ...
                        <div className="flex flex-wrap gap-2">
                          {(normalizedAnalysis.jd_match.missing_with_labels || normalizedAnalysis.jd_match.missing_keywords.map(k => ({ keyword: k, label: "Critical Gap" }))).map((item: any, i: number) => (
                            <button key={i} onClick={() => handleSkillClick(typeof item === 'string' ? item : item.keyword)} className="px-3 py-1.5 bg-red-500/5 border border-red-500/20 rounded-xl text-xs text-red-300/60 font-medium hover:bg-red-500/10 hover:border-red-500/40 hover:text-red-300 transition-all cursor-pointer">{typeof item === 'string' ? item : item.keyword}</button>
                          ))}
                        </div>
                      </div>
                    </div>
